...
\hypertarget{api-profile-001-ux83b7ux53d6ux4e2aux4ebaux8d44ux6599}{%
\subsubsection{API-PROFILE-001
获取个人资料}\label{api-profile-001-ux83b7ux53d6ux4e2aux4ebaux8d44ux6599}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{GET /api/v1/users/me/profile}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{api-profile-002-ux66f4ux65b0ux4e2aux4ebaux8d44ux6599}{%
\subsubsection{API-PROFILE-002
更新个人资料}\label{api-profile-002-ux66f4ux65b0ux4e2aux4ebaux8d44ux6599}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{PATCH /api/v1/users/me/profile}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{api-asset-001-ux4e0aux4f20ux7528ux6237ux8d44ux6e90}{%
\subsubsection{API-ASSET-001
上传用户资源}\label{api-asset-001-ux4e0aux4f20ux7528ux6237ux8d44ux6e90}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{POST /api/v1/users/me/assets}
\end{Highlighting}
\end{Shaded}
...
\item
  上传头像后自动更新 \texttt{user\_profiles.avatar\_asset\_id}。
\item
  上传背景后自动更新 \texttt{user\_preferences.background\_mode=image}
  和 \texttt{background\_asset\_id}。
\end{itemize}

\hypertarget{api-asset-002-ux8bfbux53d6ux7528ux6237ux8d44ux6e90}{%
\subsubsection{API-ASSET-002
读取用户资源}\label{api-asset-002-ux8bfbux53d6ux7528ux6237ux8d44ux6e90}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{GET /api/v1/users/me/assets/:assetId}
\end{Highlighting}
\end{Shaded}
...
\item
  只能读取归属当前用户的资源。
\item
  响应体为图片二进制，\texttt{Content-Type} 使用资源 MIME 类型。
\end{itemize}
...
前端 CSV 直接导入新表复用本接口与
\texttt{API-ROW-002}：先按用户配置创建表，再批量写入 CSV
数据；若写入失败，前端会调用删除表接口清理刚创建的新表。
...
\item
  ...
  \texttt{PRIMARY\_KEY}、\texttt{FOREIGN\_KEY}、\texttt{UNIQUE}、\texttt{CHECK\_IN}。
\item
...
\item
  ...
  \texttt{RESTRICT}、\texttt{CASCADE}、\texttt{SET\ NULL}、\texttt{NO\ ACTION}。
\item
  \texttt{column}：\texttt{string}；条件必填；\texttt{CHECK\_IN}
  使用，表示有限取值字段。
\item
  \texttt{values}：\texttt{array}；条件必填；\texttt{CHECK\_IN}
  使用，长度 1 到 50，仅支持字符串、数字或布尔值。
\end{itemize}

阶段说明：创建表接口支持可选
\texttt{PRIMARY\_KEY}、\texttt{UNIQUE}、\texttt{FOREIGN\_KEY} 和
\texttt{CHECK\_IN}。当前前端创建表弹窗先暴露字段、主键和默认值，更多索引/约束通过结构管理窗口或接口使用。
...
\item
  预览接口已支持单字段排序；JOIN、聚合、子查询和更复杂条件归入
  \texttt{API-QUERY}。
\end{itemize}

\hypertarget{api-row-002-ux65b0ux589eux6570ux636e}{%
\subsubsection{API-ROW-002
新增数据}\label{api-row-002-ux65b0ux589eux6570ux636e}}
...
rows & array & 条件必填 & 批量新增行数组，长度 1 到
\texttt{maxBatchInsertRows} \\
confirmation & object & 条件必填 & \texttt{rows} 超过 1 行时必填 \\
\bottomrule
\end{longtable}
...
\item
  批量新增由前端弹窗输入 JSON 数组或导入 CSV 文件；CSV 导入支持
  UTF-8、GBK 编码、跳过起始行数、按标题行自动匹配字段以及手动配置 CSV
  列到目标字段的映射关系。
\item
...
\hypertarget{api-row-003-ux4feeux6539ux6570ux636e}{%
\subsubsection{API-ROW-003
修改数据}\label{api-row-003-ux4feeux6539ux6570ux636e}}
...
confirmation & object & 条件必填 & \texttt{primaryKeys} 超过 1
行时必填 \\
\bottomrule
\end{longtable}
...
\item
  批量修改使用 \texttt{primaryKeys}
  定位已选行，当前前端一次修改一个非主键非自增字段。
\item
  无主键表在 V5 阶段禁止行内编辑。
\item
  单表容量达到限制时拒绝更新。
...
\hypertarget{api-row-004-ux5220ux9664ux6570ux636e}{%
\subsubsection{API-ROW-004
删除数据}\label{api-row-004-ux5220ux9664ux6570ux636e}}
...
\hypertarget{api-row-005-ux6279ux91cfux6570ux636eux64cdux4f5c}{%
\subsubsection{API-ROW-005
批量数据操作}\label{api-row-005-ux6279ux91cfux6570ux636eux64cdux4f5c}}

批量新增、基于主键列表的批量修改和基于主键列表的批量删除已经并入
\texttt{API-ROW-002} 至 \texttt{API-ROW-004}。批量写入规则如下：

\begin{itemize}
\tightlist
\item
  批量新增使用 \texttt{rows} 数组，单次最多 \texttt{maxBatchInsertRows}
  行。
\item
  批量修改和批量删除使用 \texttt{primaryKeys} 数组，单次最多
  \texttt{maxBatchWriteAffectedRows} 行。
\item
  超过单行时必须提交 \texttt{confirmation.confirmed=true}，且
  \texttt{confirmation.confirmText} 等于表名。
\item
  同一批次在事务中执行，全成功才提交；任意一行失败则回滚。基于 GUI
  条件对象的批量修改/删除归入查询构造器阶段。
\end{itemize}
...
sqlPreview & string & 后端生成的只读 SELECT
预览，值参数仍以占位符表达 \\
\bottomrule
\end{longtable}
...
返回表数量、视图数量、索引数量、数据大小、索引大小、表大小 Top
N、表行数分布和最近操作。
...
返回字段数量、主键信息、外键信息、索引信息、估算行数、空值统计、数值聚合统计和分类字段
Top N。
