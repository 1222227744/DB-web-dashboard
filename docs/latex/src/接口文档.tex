...
collation & string & 是 & utf8mb4\_unicode\_ci \\
\bottomrule
\end{longtable}
...
collation & string & 否 & utf8mb4\_unicode\_ci \\
\bottomrule
\end{longtable}
...
\hypertarget{api-row-001-ux5206ux9875ux67e5ux8be2ux6570ux636e}{%
\subsubsection{API-ROW-001
分页查询数据}\label{api-row-001-ux5206ux9875ux67e5ux8be2ux6570ux636e}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{GET /api/v1/databases/:databaseId/tables/:tableName/rows}
\end{Highlighting}
\end{Shaded}
...
\texttt{rows{[}{]}} 是字段名到字段值的对象。字段名必须存在于目标表。
...
\FunctionTok{\{}
  \DataTypeTok{"error"}\FunctionTok{:} \FunctionTok{\{}
    \DataTypeTok{"type"}\FunctionTok{:} \StringTok{"MYSQL\_CONSTRAINT\_ERROR"}\FunctionTok{,}
    \DataTypeTok{"code"}\FunctionTok{:} \StringTok{"DB\_NOT\_NULL\_VIOLATION"}\FunctionTok{,}
    \DataTypeTok{"details"}\FunctionTok{:} \StringTok{"批量插入第 3 行失败"}\FunctionTok{,}
    \DataTypeTok{"fieldErrors"}\FunctionTok{:} \OtherTok{[}
      \FunctionTok{\{}
        \DataTypeTok{"rowIndex"}\FunctionTok{:} \DecValTok{2}\FunctionTok{,}
        \DataTypeTok{"field"}\FunctionTok{:} \StringTok{"age"}\FunctionTok{,}
        \DataTypeTok{"message"}\FunctionTok{:} \StringTok{"字段 age 不允许为空"}
      \FunctionTok{\}}
    \OtherTok{]}\FunctionTok{,}
    \DataTypeTok{"mysql"}\FunctionTok{:} \FunctionTok{\{}
      \DataTypeTok{"errno"}\FunctionTok{:} \DecValTok{1048}\FunctionTok{,}
      \DataTypeTok{"sqlState"}\FunctionTok{:} \StringTok{"23000"}\FunctionTok{,}
      \DataTypeTok{"sqlMessage"}\FunctionTok{:} \StringTok{"Column \textquotesingle{}age\textquotesingle{} cannot be null"}
    \FunctionTok{\}}
  \FunctionTok{\}}
\FunctionTok{\}}
\end{Highlighting}
...
\item
  行内编辑必须优先携带 \texttt{primaryKey}，后端使用主键定位唯一行。
\item
...
\tightlist
\item
  使用 \texttt{where} GUI 条件对象。
\item
  不允许无条件批量修改。
\item
  影响行数超过阈值时必须携带 \texttt{confirmation}。
\end{itemize}
...
\begin{Highlighting}[]
\FunctionTok{\{}
  \DataTypeTok{"code"}\FunctionTok{:} \DecValTok{412}\FunctionTok{,}
  \DataTypeTok{"message"}\FunctionTok{:} \StringTok{"删除将影响关联表数据，请确认"}\FunctionTok{,}
  \DataTypeTok{"data"}\FunctionTok{:} \KeywordTok{null}\FunctionTok{,}
  \DataTypeTok{"traceId"}\FunctionTok{:} \StringTok{"01HZ0000000000000000000000"}\FunctionTok{,}
  \DataTypeTok{"error"}\FunctionTok{:} \FunctionTok{\{}
    \DataTypeTok{"type"}\FunctionTok{:} \StringTok{"CONFIRMATION\_REQUIRED"}\FunctionTok{,}
    \DataTypeTok{"code"}\FunctionTok{:} \StringTok{"CONFIRMATION\_CASCADE\_DELETE"}\FunctionTok{,}
    \DataTypeTok{"source"}\FunctionTok{:} \StringTok{"server"}\FunctionTok{,}
    \DataTypeTok{"details"}\FunctionTok{:} \StringTok{"删除 students 中 1 行将级联影响 scores 中 5 行"}\FunctionTok{,}
    \DataTypeTok{"fieldErrors"}\FunctionTok{:} \OtherTok{[]}\FunctionTok{,}
    \DataTypeTok{"mysql"}\FunctionTok{:} \KeywordTok{null}
  \FunctionTok{\}}
\FunctionTok{\}}
\end{Highlighting}
\end{Shaded}
