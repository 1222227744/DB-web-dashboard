...
当前项目已具备基础登录注册、个性化工作台、数据库管理、表结构管理、表数据
CRUD 和 GUI 查询能力，后续目标是在本地 MySQL
之上继续完善统计图表和更完整的审计体验。系统不重新实现数据库内核，而是在本地
...
V6 & GUI 查询 & 已落地普通查询构造器、结构化 AST
接口、JOIN、聚合、分组、HAVING、排序、分页、高级 AST
子查询入口和表格预览选择体验收尾 \\
V7 & 统计与 DIY &
用户级、数据库级、表级统计图表，审计日志列表，Dashboard
卡片显示/隐藏/排序和布局偏好 \\
V5+ & 前期遗留补齐 &
...
\hypertarget{v7-ux5f00ux53d1ux51c6ux5907}{%
\subsubsection{8.1 V7 开发准备}\label{v7-ux5f00ux53d1ux51c6ux5907}}

V7 的目标是在不破坏现有数据库管理、表结构管理、表数据 CRUD 和 GUI
查询闭环的前提下，把首页升级为真正有信息密度的统计型
Dashboard，并给用户提供轻量 DIY 能力。
...
\tightlist
\item
  纳入：统计接口、审计列表、首页统计图表、数据库级统计入口、表级统计入口、Dashboard
  卡片显示/隐藏/排序、布局偏好持久化。
\item
  不纳入：自由拖拽网格、任意自定义卡片脚本、多套布局方案、后台定时统计任务。
\item
  性能原则：优先使用 \texttt{information\_schema}
  和审计表聚合；表级统计需要限制字段数量、Top N 数量、采样或执行时间。
\end{itemize}
...
\endhead
V7.1 & 统计与审计后端基础 &
完成用户级、数据库级、表级统计接口和审计日志列表接口 \\
V7.2 & 首页统计卡片 &
首页展示用户总览、数据库分布、操作趋势和最近操作 \\
V7.3 & 工作台统计入口 &
数据库工作台展示数据库级统计，表结构窗口提供表级统计入口 \\
V7.4 & Dashboard DIY & 支持卡片显示/隐藏/排序，并保存到用户偏好 \\
V7.5 & 收尾验收 &
加载、空状态、错误态、容量限制、响应式和构建检查全部通过 \\
\bottomrule
\end{longtable}
