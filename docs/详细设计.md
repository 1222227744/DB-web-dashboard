...
- `POST /api/v1/databases/:databaseId/tables/:tableName/rows`：新增单行。
- `PATCH /api/v1/databases/:databaseId/tables/:tableName/rows`：基于主键更新单行。
- `DELETE /api/v1/databases/:databaseId/tables/:tableName/rows`：基于主键删除单行。
...
- 创建表阶段支持字段、可选主键、唯一约束和普通/唯一索引的服务端校验与 DDL 生成。
- 外键、检查约束、字符串有限取值约束、枚举式约束、视图管理和在线修改表结构作为后续增强逐步落地。
- 结构管理窗口作为后续字段增删改、索引管理、约束管理的统一入口；V4 先完成读取与入口占位。
